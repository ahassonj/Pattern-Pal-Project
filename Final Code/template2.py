# File for second pattern template of stars that is an instance of the user defined mode mode object.
...
class patternTemplate2(buildPattern):
    ...
    def drawBaseGrid(mode, canvas):
        ...
        drawStar1(canvas, point(cx - rad, cy - rad), point(cx + rad, cy + rad), mode.starScale, mode.userColor) 
        drawStar2(canvas, point(cx - rad, cy - rad), point(cx + rad, cy + rad), mode.userColor)
    ...
    def mouseDragged(mode, event):
        if not mode.drawUserPattern:
            if mode.createCircle:
            ...
            elif mode.changeSlider:
                # dragging the mouse adjusts the slider for this template in order to adjust and skew the template
                if abs(event.x - mode.sliderLocation.x) <= 10 and abs(event.y - (mode.sliderLocation.y)) <= 25:
                    if 40 <= event.y <= (mode.height - 20):
                        ...
                        mode.starScale = mapRange(event.y, 20, mode.height - 20, -100, 100)
            elif mode.freeDraw:
...
# all grid functions are the same as those found in the userDefinedPattern file
    ...
    def standardGrid(mode, canvas, startPoint, endPoint, level):
        ...
        if level == 0:
            ...
            drawStar1(canvas, startPoint, endPoint, scale, mode.userColor) 
            drawStar2(canvas, startPoint, endPoint, mode.userColor)
        else:
    ...
    def spiralGrid(mode, canvas, startPoint, endPoint, level, rad, rotationAngle):
        ...
        if level == 0:
            ...
            drawStar1(canvas, point(cx - rad, cy - rad), point(cx + rad, cy + rad), scale, mode.userColor) 
            drawStar2(canvas, point(cx - rad, cy - rad), point(cx + rad, cy + rad), mode.userColor)
        else:
            drawStar1(canvas, point(cx - rad, cy - rad), point(cx + rad, cy + rad), scale, mode.userColor) 
            drawStar2(canvas, point(cx - rad, cy - rad), point(cx + rad, cy + rad), mode.userColor)
